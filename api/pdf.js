...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Single logo card — full width, dark background
    const cardY = 42;
    const cardH = H - 150;
    p3.drawRectangle({ x: 40, y: cardY, width: W - 80, height: cardH, color: bgDark, borderRadius: 8 });
    if (logoImg) {
      const ld = logoImg.scaleToFit(W - 200, cardH - 80);
      p3.drawImage(logoImg, { x: (W - ld.width) / 2, y: cardY + (cardH - ld.height) / 2, width: ld.width, height: ld.height });
    }
